...
// warm AND actively serving a request. When a long generation chain (e.g.
// 28 chapters in one approve request) exceeds the function timeout, the
// instance freezes mid-job and — with nothing to re-drive the queue — all
// remaining chapters sit at "Pending" forever (the exact stall reported after
// chapter 6).
//
...
//      instance. It loops in-invocation too, until `deadlineMs` approaches the
//      function's maxDuration.
//   4. vercel.json schedules it once a day as a backstop for dead chains.
//
...
export const maxDuration = 60; // Pro allows 60s+; run jobs until ~deadlineMs
 ...
 * freezes. Uses an absolute URL (browser/server never calls localhost here;
 * this runs server-side and Vercel routes internal hostnames correctly).
 */
