...
class PersistentJobQueue {
  ...
  async startJob(jobId: string, jobType: JobType): Promise<void> {
    ...
    // survives instance freezes; locally (no CRON_SECRET) use the in-process
    // loop directly.
    if (isServerless()) {
      kickQueuePump();
    } else {
  ...
  private async claimNextJob(): Promise<{
    ...
    return this.withTransactionRetry(
      async (tx) => {
        ...
        // meantime the update affects 0 rows and we move on to the next
        // candidate — this prevents two serverless invocations from running
        // the same job twice.
        ...
        if (claimed.count === 0) {
          ...
          // the caller/pump will simply find no more work or retry on the next
          // pass (transactions are retried, and a lost claim is expected).
          return null;
        }
