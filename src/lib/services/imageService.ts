...
/**
 * Backend 2: Pollinations AI (flux by default).
...
export async function generateColoringPage(bookId: string, ownerId: string, chapterIndex: number, subject: string, theme?: ColoringTheme | null): Promise<GeneratedImageResult> {
  ...
  const prefix = themeConfig ? `${themeConfig.pagePromptPrefix} ` : 'Coloring book page:'
  const prompt = `${prefix}${subject}. ${adultStyleAddon}. Pure black and white line art, clean white background, no shading, no grayscale, no color, high contrast.`
