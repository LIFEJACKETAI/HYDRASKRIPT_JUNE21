...
// Includes text chunking, voice mapping, and playable-audio conversion.
...
/** Gemini TTS config. Audiobooks never reuse the text-generation provider key. */
 ...
 * stylistic defaults rather than Google Cloud voice ids. Fish Audio voice
 * selection is driven by `FISH_AUDIO_REFERENCE_ID`, not this mapping.
 */
 ...
 * Primary provider. Returns base64 MP3 audio.
 *
 * https://api.fish.audio/v1/tts
 * Content-Type: application/json, model sent as a request header.
 */
export async function generateAudioChunkWithFishAudio(text: string): Promise<AudioChunkResult> {
  const apiKey = process.env.FISH_AUDIO_API_KEY;
  const referenceId = process.env.FISH_AUDIO_REFERENCE_ID;
  const model = process.env.FISH_AUDIO_MODEL?.trim() || 's2.1-pro';
...
export async function generateAudioChunkWithGemini(
  text: string,
  voiceId: string
): Promise<{ success: boolean; audioBase64?: string; audioMimeType?: string; provider?: string; error?: string }> {
  try {
    return await withRetry(async () => {
 ...
 */
export async function generateAudioChunk(text: string, voiceId: string): Promise<AudioChunkResult> {
