...
export function RecoveryHandler() {
  ...
  useEffect(() => {
    // Built here, not in the component body: this component is rendered by the
    // ROOT layout, so a render-time `createClient()` also ran while Next
    // prerendered every static page (/_not-found included) - and an unconfigured
    // env made that throw and fail the whole build.
    ...
    handleRecovery();
    // `supabase` is created inside the effect on purpose (see above).
  }, [router, pathname]);
