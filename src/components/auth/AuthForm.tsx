...
export default function AuthForm() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    alert('Auth button clicked! Starting process...');
    console.log('[Auth] Attempting authentication...', { email, isSignUp });
    setLoading(true);

    try {
      if (!supabase) {
        throw new Error('Supabase client not initialized. Check your NEXT_PUBLIC environment variables.');
      }
    ...
    <div className="w-full max-w-md p-8 rounded-2xl bg-[#0d0d10] border border-white/10 shadow-2xl">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-slate-400">
          {isSignUp
            ? 'Start your AI publishing journey today.'
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="relative my-4">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-[#0d0d10] px-2 text-slate-500">Or continue with email</span>
          </div>
        </div>
